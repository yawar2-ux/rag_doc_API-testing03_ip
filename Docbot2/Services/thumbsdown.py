...
class ThumbsDownService:
    """Service for managing persistent Chroma DB collections for thumbsdown."""

    def __init__(self, base_dir: str = "chroma_thumbsdown"):
        """Initialize the ThumbsDown service.
    ...
    def query_collection(self, collection_name: str, query_text: str, k: int = 1, score_threshold: float = 0.5) -> List[Dict[str, Any]]:
        ...
        collection_dir = self.base_dir / sane_collection_name
        if not collection_dir.exists():
            print(f"ThumbsDown Query: Collection directory {collection_dir} not found.")
            return []

        try:
            collection = self.get_collection(sane_collection_name)
            if self.get_collection_count(sane_collection_name) == 0:  # Use the robust count method
                print(f"ThumbsDown Query: Collection {sane_collection_name} is empty.")
                return []
...
thumbsdown_service = ThumbsDownService()